...
g = open("trip_data_1/weightEpsilonGreedy_" + fname + ".csv", "w+")
for test in range(1, 931):
    ...
    for line in f:
        ...
        start_time = datetime.strptime(vals[5], "%Y-%m-%d %H:%M:%S")
        end_time = datetime.strptime(vals[6], "%Y-%m-%d %H:%M:%S")
        ...
        cur_T = int(time_in_seconds)/300 + 1
        ...
        er = random.uniform(0, 1)
        cur_u = -1
        if er < 1-epsilon: #LP algorithm
            r = random.uniform(0, 1)
            cur_sum = 0
            for val in free_lhs:
                temp_u = Xopt[cur_T][val][cur_v]
                if (r - cur_sum) <= temp_u/p_v:
                    cur_u = val
                    break
                cur_sum += temp_u/p_v
        ...
        for key in avCarsMap:
            if avCarsMap[key]["lhs_num"] == cur_u:
                ...
                avCarsMap[key]["end"] = start_time + timedelta(seconds=avCarsMap[key]["len"])
